...
consisting of a ribose, one or more phosphate groups and a
nucleobase\footnotemark\\\marginfig{nucleotide}}]{nucleotides}\footnotetext{{Figure
adapted from
\url{https://commons.wikimedia.org/w/index.php?title=File:Nucleotides_1.svg&oldid=128814238}}}.
The chemical nature of nucleotides means that these will easily polymerase into
long, relatively stable chains. \dna is made up of four different types of
nucleotides: adenine (\nA), cytosine (\nC), guanine (\nG) and thymine (\nT).
Thus, \dna can be thought of as a long string of four different letters, and
that is indeed how it is often represented. Text is written from left to right
in Western cultures. \dna is written from \fivep to \threep.\footnote{Referring
to the number of the phosphate atom in each nucleotide, which is exposed at
...
Proteins, finally, are the main effectors of cell function. Their abundance is
mainly governed by the abundance of \mrna[s]
\citep{Li:2014,Jovanovic:2015,Csardi:2014}.
...
acid to form a \define[\defineword{peptide bond} a covalent bond formed between
a carboxyl and an amino group: \ce{COOH + NH2 -> CO-NH + H2O}]{peptide bond}
\citep{Alberts:2002}.
