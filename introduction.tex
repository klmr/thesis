...
adapter molecules are called \define[{
    ...
    for asparagine (Asn). The \abbr{trna} carries the anticodon \anticodon{GUU}
    which sits in the middle of the \define{anticodon loop}, highlighted in
    \tertiaryname{}. Annotated in \primaryname{} and \secondaryname{} are two
    structures called the \define{D arm} and \define{T arm}. After
    transcription, the precursor \trna undergoes chemical modification of some
    of its bases, which is not shown here. Structure predicted by \name{COVE}
...
the \define{A box} and \define{B box}, respectively. The A box starts about
...
box motif. \tfiiic leads to the binding of the \pol3 recruitment factor \tfiiib
upstream of the \trna gene. A subunit of \tfiiib, the \tbp, binds to an upstream
\seq{TATA} motif, if present. Although the presence of this motif is not
...
Bindin of \tfiiib in turn leads to the association of \pol3 with the gene body,
and the initiation of transcription. Transcription stops when \pol3 encounters a
...
it requires \define{translational recoding}, an altogether different translation
...
an \define{anticodon isoacceptor family}. \trna genes for the same amino acid
form an \define{amino acid isotype}.

To avoid confusion between the two, codons in this thesis will always be typeset
like this: \fwdstrand{\codon{CAU}}; anticodons will always be typeset like this:
