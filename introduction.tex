...
Using the above example, and assuming the gene expression counts \(A: 10\) and
\(B: 20\) counts, we can summarise the codon usage as follows:
...
\begin{table}[H]
    ...
    \begin{subtable}{0.5\textwidth}
        \centering
        \begin{tabu} to \textwidth
            {@{}>{\collectcell\codon}l<{\endcollectcell}SSS@{}}
            \toprule
            \multicolumn{1}{@{}l}{Codon} & \(A\) & \(B\) & {\(A + B\)} \\
            \midrule
            AAA & 3 & 1 & 4\tabularnewline
            AAG & 1 & 3 & 4\tabularnewline
            AUG & 2 & 3 & 5\tabularnewline
            GAC & 4 & 0 & 4\tabularnewline
            GAU & 1 & 3 & 4\tabularnewline
            UAC & 2 & 7 & 9\tabularnewline
            \bottomrule
        \end{tabu}
    ...
    \begin{subtable}{0.5\textwidth}
        ...
        \begin{tabu} to \textwidth
            ...
            \multicolumn{1}{@{}l}{Codon} & \(A\) & \(B\) & {\(A + B\)} \\
            \midrule
            AAA & 30 & 20 & 50\tabularnewline
            AAG & 10 & 60 & 70\tabularnewline
            AUG & 20 & 60 & 80\tabularnewline
            GAC & 40 & 0 & 40\tabularnewline
            GAU & 10 & 60 & 70\tabularnewline
            UAC & 20 & 140 & 160\tabularnewline
            \bottomrule
        \end{tabu}
        \caption{Codon usage weighted by gene expression}
    \end{subtable}%
    \tabcap{codon-usage}{Example codon usage.}{}
\end{table}

Note that we have omitted the stop codon, but not the start codon, since the
latter corresponds to a \trna isoacceptor, while the former does not. The column
\(A+B\) shows how the codon usage of a transcriptome consisting of these two
genes would look like. In the following chapters, we will rarely look at the
codon usage of single transcripts, and rather at either the sum codon usage or
the mean codon usage of gene sets.
