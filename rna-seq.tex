...
\begin{enumerate*}
    ...
    \item Cross-hybridisation causes low-level, non-specific binding of
        ...
        cannot be compared.\todo{John suggests removing b/c still hard with RNA-seq; but we do this in our studies}
    \item Several interesting biological questions which cannot be answered by
        ...
        using \abbr{rnaseq}; these include the analysis of isoforms from
...
\textfig{rna-seq-workflow}{body}{0.8\textwidth}
...
\(\sigma^2_{ij}\).\todo{Formulas for \(\mu\), \(\sigma^2\)}
